...
import gazpromNeftekhimSalavatLogo from "@/imports/clients/new/gazprom-neftekhim-salavat.webp";
import niihimmashLogo from "@/imports/clients/new/niihimmash.webp";
import bashkirGenerationLogo from "@/imports/clients/new/bashkir-generation.webp";
import nstecLogo from "@/imports/clients/new/nstec.webp";
import uraltekhnostroyLogo from "@/imports/clients/new/uraltekhnostroy.webp";
import roskhimLogo from "@/imports/clients/new/roskhim.webp";
import rmzLogo from "@/imports/clients/new/rmz.webp";
...
const clients = [
  ...
  { name: "Газпром нефтехим Салават", logo: gazpromNeftekhimSalavatLogo },
  { name: "НИИхиммаш", logo: niihimmashLogo },
  { name: "Башкирская генерирующая компания", logo: bashkirGenerationLogo },
  { name: "НСТЭК", logo: nstecLogo },
  { name: "Уралтехнострой", logo: uraltekhnostroyLogo },
  { name: "Росхим", logo: roskhimLogo },
  { name: "РМЗ", logo: rmzLogo },
];
...
const pipelineSlides: ProjectSlide[] = [
  {
    images: [pipelineN59A2356, pipelineN59A2349, pipelineN59A2333],
    layout: "collage",
  },
...
const projects = [
  {
    ...
    slides: flareSlides,
    description:
      "Факельные установки предназначены для сброса и сжигания горючего газа и жидкостей. Применяются на нефтегазодобывающих, нефтеперерабатывающих и химических предприятиях для нейтрализации постоянных, периодических и аварийных сбросов, предотвращая их выброс в атмосферу.",
    clients:
      "АО «Конденсат» (Республика Казахстан), ООО «НПП «Нефтегазинжиниринг», ООО «Башнефть-Полюс», ООО «Иркутская нефтяная компания», АО «Норильсктрансгаз», ООО «ЛУКОЙЛ-ПЕРМЬ», ООО «Сибур» и другие.",
  },
  {
    title: "Элементы трубопроводов",
    slides: pipelineSlides,
    description:
      "Являются узлами технологических трубопроводов, предназначенных для комплектации, сборки и реконструкции технологических трубопроводов DN 50 – DN 1400.",
    clients:
      "ООО «Газпромкомплектация» для газоперерабатывающего комплекса в составе комплекса переработки этансодержащего газа в районе посёлка Усть-Луга.",
  },
  {
    title: "Смесители",
    ...
    description:
      "Предназначен для смешивания основного конденсата. Устанавливается в помещении машинного зала на трубопроводе основного конденсата между ПНД-5 и питательным насосом первой ступени (ПЭН-1) перед подводом силовой воды от ПГТН турбоустановки К-306-15,7/50 с реакторной установкой БРЕСТ-ОД-300.",
  ...
  },
  {
    ...
    description:
      "Являются элементом паровых стационарных котлов. Предназначены для перегрева поступающего в котел насыщенного пара.",
...
function ProjectSlideMedia({
  ...
  return (
    <div
      ...
      style={{
        scrollbarWidth: "none",
        gridTemplateColumns: `repeat(${slide.images.length}, minmax(0, 1fr))`,
      }}
    ...
    >
      {slide.images.map((image, imageIndex) => (
        <div
          key={image}
          className="relative h-full min-w-full snap-center overflow-hidden border-r-2 border-[#263740] last:border-r-0 lg:min-w-0"
        >
          <img
            src={image}
...
export function ProjectsPreview() {
  ...
  useEffect(() => {
    const maxSlides = Math.max(
      ...projects.map(
        (project) => getVisibleSlides(project.slides, isMobile).length,
      ),
    );
    ...
    <section className="relative overflow-hidden bg-[#263740] px-6 py-20 lg:px-12 lg:py-28">
      ...
      <div className="mx-auto max-w-[1920px]">
        <div className="mb-12 flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            <div className="mb-5 flex items-center gap-4 text-xs font-semibold uppercase tracking-[0.18em] text-[#91a0a8]">
              <span className="h-px w-14 bg-[#50626c]" />
              Практика и опыт
            </div>
            <h2 className="max-w-4xl text-[clamp(2.25rem,5vw,5rem)] font-semibold leading-[0.98] tracking-[-0.045em] text-white">
          ...
          {projects.map((project, index) => {
            ...
            return (
              <motion.article
                key={project.title}
                layout
                className={`group relative min-h-[460px] overflow-hidden bg-[#50626c] lg:min-h-[520px] ${index === 0 || index === 3 ? "lg:col-span-7" : "lg:col-span-5"}`}
              >
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className="absolute inset-0 transition-transform duration-1000 group-hover:scale-[1.025]"
                  >
                    <ProjectSlideMedia
                      slide={visibleSlides[slideIndex % visibleSlides.length]}
                      alt={project.title}
                    />
                  </motion.div>
                </AnimatePresence>
                ...
                <div className="absolute left-7 top-7 text-5xl font-light tracking-[-0.05em] text-white/25 sm:left-9 sm:top-9">
                  0{index + 1}
                </div>
                <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-5 p-7 sm:p-9">
                  ...
                  <button
                    ...
                    className="flex h-14 w-14 shrink-0 items-center justify-center bg-[#8D9DA6] text-white transition hover:bg-white hover:text-[#50626c]"
                  >
                    <ArrowUpRight size={22} />
                  </button>
                </div>
                <div
                  className="absolute inset-x-0 bottom-0 h-[3px] origin-left bg-[#71838c] transition-transform duration-[5000ms]"
                  style={{
                    transform: `scaleX(${((slideIndex % visibleSlides.length) + 1) / visibleSlides.length})`,
                  }}
                />
              </motion.article>
            );
          ...
          {activeProject !== null && (
            ...
            >
              <button
                ...
                className="absolute inset-0 h-full w-full cursor-default bg-[#17242b]/70 backdrop-blur-md"
              />
              <motion.aside
                role="dialog"
                aria-modal="true"
              ...
              >
                <div className="relative min-h-[40vh] overflow-hidden bg-[#50626c]">
                  ...
                  <button
                    ...
                    className="absolute right-5 top-5 z-10 flex h-12 w-12 items-center justify-center border border-white/40 bg-[#263740]/35 text-white backdrop-blur-md transition hover:bg-white hover:text-[#50626c]"
                  >
                    <X size={22} />
                  </button>
                  <div className="absolute inset-x-0 bottom-0 p-7 text-white sm:p-10">
                    <div className="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-white/65">
                ...
                <div className="flex-1 p-7 text-[#40515a] sm:p-10">
                  ...
                  <p className="mb-9 text-lg leading-relaxed">
        ...
        <div className="relative mt-20 border-t border-white/15 pt-9">
          ...
          <div className="grid grid-cols-2 items-stretch gap-3 lg:grid-cols-4">
            {clients.map((client) => (
              <div
                key={client.name}
                className="group relative flex h-28 w-full items-center justify-center overflow-hidden border border-white/10 bg-white px-5 transition duration-500 hover:-translate-y-1 hover:border-[#71838c]/80 sm:h-32 lg:h-34 xl:h-36"
              >
                <span className="absolute bottom-0 left-0 h-[3px] w-0 bg-[#50626c] transition-all duration-500 group-hover:w-full" />
                <img
                  src={client.logo}
                  alt={client.name}
                  className="max-h-16 w-full max-w-56 object-contain transition-transform duration-500 group-hover:scale-105 sm:max-h-20 lg:max-h-24"
                  loading="lazy"
                />
              </div>
            ))}
